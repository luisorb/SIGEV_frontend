...
const STEP_FIELDS: Record<StepKey, (keyof EventFormValues)[]> = {
  datos: ['numeroEvento', 'sufijo', 'responsable', 'dependencia', 'fechaEvento', 'asistentes', 'dias', 'programa', 'tipoEvento', 'instanciaParticipacion'],
      ...
      {stepIndex === 0 && (
        ...
        >
          ...
          <Field
            ...
            required
            hint="Instancia en la que se desarrollará la participación."
            error={errors.instanciaParticipacion?.message}
          >
            <select
              id="instanciaParticipacion"
              {...register('instanciaParticipacion')}
              className={field('instanciaParticipacion')}
            >
              <option value="">Seleccionar instancia</option>
              {INSTANCIAS_CONSEJOS.map((i) => (
