      ...
      <div className="shrink-0 flex flex-wrap items-center gap-3">
        ...
        {showFilters && (
          <>
            ...
            <select value={filters.desembolsoId} onChange={(e) => updateFilter('desembolsoId', e.target.value)} className="px-2.5 py-1.5 border border-slate-300 rounded-lg text-xs focus:ring-2 focus:ring-primary focus:border-transparent bg-white"><option value="">Todos los recursos disponibles</option>{Object.entries(desembolsosMap).map(([id, nombre]) => (<option key={id} value={id}>{nombre}</option>))}</select>
